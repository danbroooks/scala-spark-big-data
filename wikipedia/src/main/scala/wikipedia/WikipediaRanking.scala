...
object WikipediaRanking {
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val matches = for { article <- rdd; lang <- langs.filter(article.mentionsLanguage) } yield (lang, article)
    matches.groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val matches = for { article <- rdd; lang <- langs.filter(article.mentionsLanguage) } yield (lang, 1)
    matches.reduceByKey(_ + _).collect().toList.sortBy(_._2).reverse
  }
